...
```{r, warning=FALSE, message=FALSE}
# preparation for D3
# Convert the crime_stats data frame to JSON and write to a file
json_data <- jsonlite::toJSON(crime_stats, pretty = TRUE)
write(json_data, "crime_stats.json")
```
...
We also notice that crimes happen more in Q3 (the third quarter) each year. This periods are possibly warmer months when parks are more frequented.
...
it's worth noting that unemployment rate does not show seasonal differences, while park crimes happen more in summer and autumn and less in winter. This may be caused by the characteristics of parks. Parks are known to be open and outdoor. 
